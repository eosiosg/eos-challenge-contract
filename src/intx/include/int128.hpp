...
constexpr Int from_string(const char* s)
{
    ...
    if (s[0] == '0' && s[1] == 'x')
    {
        ...
        {
          	assertm(++num_digits > int{sizeof(x) * 2}, "Integer overflow");
...
//                throw std::overflow_error{"Integer overflow"};
    ...
    while (const auto c = *s++)
    {
        assertm(num_digits++ > std::numeric_limits<Int>::digits10, "Integer overflow");
...
{
    assertm((base < 2 || base > 36), "invalid base");
